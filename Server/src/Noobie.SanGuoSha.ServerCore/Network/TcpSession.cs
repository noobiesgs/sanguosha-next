...
public abstract class TcpSession<TSession> : IDisposable where TSession : TcpSession<TSession>
{
    /// <summary>
    /// Session Id
    /// </summary>
    ...
    /// Server
    /// </summary>
    public TcpServer<TSession>? Server { get; private set; }
    ...
    internal void Init(TcpServer<TSession> server, int id)
    {
        Id = id;
        Server = server;
    ...
    /// </summary>
    /// <param name="buffer">Buffer to send as a sequence of bytes</param>
    /// <returns>'true' if the data was successfully sent, 'false' if the client is not connected</returns>
    public virtual bool SendAsync(ReadOnlySequence<byte> buffer)
    {
        if (!IsConnected)
            return false;
    ...
    protected virtual void Dispose(bool disposingManagedResources)
    {
        ...
        if (!IsDisposed)
        {
            if (disposingManagedResources)
            {
                ...
                _sendBufferMain.Dispose();
                _sendBufferFlush.Dispose();
            }
