...
public abstract class TcpServer<TSession> : IDisposable where TSession : TcpSession<TSession>
{
    private readonly NumberSpaceInt32 _numberSpace;

    /// <summary>
    /// Initialize TCP server with a given endpoint
    /// </summary>
    protected TcpServer()
    {
        _numberSpace = new NumberSpaceInt32();
    }
    ...
    public virtual bool Start(EndPoint endpoint)
    {
    ...
    private void ProcessAccept(SocketAsyncEventArgs e)
    {
        if (e.SocketError == SocketError.Success)
        {
            ...
            session.Init(this, GenerateSessionId());
    ...
    // Server sessions
    protected readonly ConcurrentDictionary<int, TSession> Sessions = new();
    ...
    public TSession? FindSession(int id)
    {
        // Try to find the required session
        return Sessions.TryGetValue(id, out TSession result) ? result : null;
    }
    ...
    internal void RegisterSession(TSession session)
    {
        // Register a new session
    ...
    {
        // Unregister session by Id
        Sessions.TryRemove(id, out TSession _);
    }

    public TSession[] GetAllSessions()
    {
    ...
    protected virtual void OnConnecting(TcpSession<TSession> session) { }
    /// <summary>
    /// Handle session connected notification
    /// </summary>
    /// <param name="session">Connected session</param>
    protected virtual void OnConnected(TcpSession<TSession> session) { }
    /// <summary>
    /// Handle session disconnecting notification
    /// </summary>
    /// <param name="session">Disconnecting session</param>
    protected virtual void OnDisconnecting(TcpSession<TSession> session) { }
    /// <summary>
    /// Handle session disconnected notification
    /// </summary>
    /// <param name="session">Disconnected session</param>
    protected virtual void OnDisconnected(TcpSession<TSession> session) { }
    ...
    internal void OnConnectingInternal(TcpSession<TSession> session) { OnConnecting(session); }
    internal void OnConnectedInternal(TcpSession<TSession> session) { OnConnected(session); }
    internal void OnDisconnectingInternal(TcpSession<TSession> session) { OnDisconnecting(session); }
    internal void OnDisconnectedInternal(TcpSession<TSession> session) { OnDisconnected(session); }
